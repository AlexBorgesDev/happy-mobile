...
const UserSavedPosts = ({ navigation }: UserPostsProps) => {
  ...
  const handleGetUserPosts = async (p = page, refresh?: boolean) => {
    ...
    try {
      const data = await userService.get(p);
      if (!data) return;
      ...
      snackActions.setMessage('Algo deu errado ao obter seus posts salvos');
    }
  ...
  return (
    <Container>
      <Appbar.Header statusBarHeight={StatusBar.currentHeight}>
        ...
        <Appbar.Content title="Post Salvos" />
      </Appbar.Header>
          ...
          <>
            ...
            {!loading && finished && (
              <>
                <FinishedText>
                  {posts.length > 0
                    ? 'Esses são todos os seus posts salvos.'
                    : 'Ops! Você ainda não salvo nenhum post.'}
                </FinishedText>
